...
class Graph(object):
    ...
    def all_linked(self):
        ...
        for i in range(len(vertices)):
            for e in range(i + 1, len(vertices)):
                if self.find_path(vertices[i], vertices[e]) is None:
                    return False
        return True
...
def find_min_required(matrix):
    g, missing = matrix_to_dict(matrix)
    print(g)
...
print(find_min_required(matrix))
